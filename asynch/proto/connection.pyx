...
# single Connection safe to share between threads.
import asyncio
import logging
import ssl
import warnings
...
class Connection:
    ...
    def __init__(  # nosec:B107
        ...
        **kwargs,
        ...
        if self.settings.pop("use_numpy", False):
            # Accepted-and-ignored for backward compatibility: it was stored but
            # never implemented — numpy was not even imported.
            warnings.warn(
                "The 'use_numpy' setting is not supported and has no effect",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def unexpected_packet_message(self, expected, packet_type):
        ...
        return (
            f"Unexpected packet from server {self.get_server()} "
            f"(expected {expected}, got {packet_type})"
        )
    ...
    async def _init_connection(self, host, port):
